...
class RenderIndividualSoul {
    ...
    public NameText(): JSX.Element {
        return (
        <h6 className="big-text">{this.soul.name + " "}
            {
    ...
    public genStatText(dict: StatDict): JSX.Element {
        return <span>
            {Object.keys(this.soul.stats).filter((i) => i !== "HP").map((key) => {
    ...
    public genSkillInfo(): JSX.Element {
        return (
            <div className="skillDiv">
                <hr/>
                <ul>
                    {this.soul.skills.map((skill: Skill) => <li>{skill.data.name}</li>)}
                </ul>
            </div>
        );
    }
...
export {
    RenderIndividualSoul, RenderPlayerSoul
}