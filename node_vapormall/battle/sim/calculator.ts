...
class Calculator {
        ...
        switch (skill.data.meta.category) {
            ...
            case CONSTANTS.SKILLCATEGORIES.GLITCHDAMAGE:
                if (skill.data.power !== null) {
                    let damage = this.damageCalc(
                        user,
                        skill,
                        target,
                        skill.data.meta.category ===
                            CONSTANTS.SKILLCATEGORIES.GLITCHDAMAGE
                    );

                    let multiplier = 1;
                    target.soul.soul_species.types.forEach((type) => {
                        multiplier *= this.typeMultiplier(
                            skill.data.type,
                            type
                        );
                    });
                    ...
                    if (multiplier === 0) {
                        ...
                        if (skill.data.meta.drain !== 0) {
                            const drain = this.capDamageToHP(
                                Math.floor(
                                    damage * (skill.data.meta.drain / 100)
                                ),
                                user.soul
                            );
                ...
                skill.data.stat_changes.forEach((statChange: StatChange) => {
                    if (statChange.stat === CONSTANTS.STATS.HP) {
                        ...
                        if (statChange.change > 1) {
                            changeDesc +=
                                " " + Math.abs(statChange.change) + " stages!";
                        } else {
                            changeDesc +=
                                " " + Math.abs(statChange.change) + " stage!";
                        }
    ...
    damageCalc(
        ...
        if (isGlitch) {
            const damage_num =
                user.calculateStat(CONSTANTS.STATS.GLITCHATTACK) *
                skill.data.power!;
            const damage = Math.ceil(
                damage_num / target.calculateStat(CONSTANTS.STATS.GLITCHDEFENSE)
            );
            return damage;
        }

        const damage_num =
            user.calculateStat(CONSTANTS.STATS.ATTACK) * skill.data.power!;
        const damage = Math.ceil(
            damage_num / target.calculateStat(CONSTANTS.STATS.DEFENSE)
        );
        return damage;
    }
    ...
    typeMultiplier(
        attackType: CONSTANTS.TYPES,
        defendType: CONSTANTS.TYPES
    ): number {
        switch (attackType) {
    ...
    checkFaint(soul: BattleSoul): void {
        if (soul.soul.currentHP <= 0) {
            this.battle.messageTimer.endMessageBlock();
            this.battle.messageTimer.addMessage(
                soul.renderer.getName() + " was destroyed!"
            );

            this.battle.allSouls().filter((s) => {
                s === null || s.soul.name !== soul.soul.name;
            });
            if (soul instanceof FieldedPlayerSoul) {
                ...
                this.battle.enemySouls.splice(
                    this.battle.enemySouls.indexOf(soul as EnemySoul),
                    1
                );
            }
